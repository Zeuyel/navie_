...
from navie.utils.logger import setup_logger
from navie.utils.wmemail_provider import WMEmailProvider, WMEmailAccount
...
class EmailManagerFactory:
    ...
    def load_from_config(config_file="email_config.json", account_index=None):
        ...
        async def load_from_db(target_account_index):
            try:
                try:
                ...
                accounts = await conn.fetch("""
                    SELECT email, password, client_id, access_token
                    FROM email_accounts
                ...
                if target_account_index is None:
                    target_account_index = current_index

                if target_account_index >= len(accounts):
                    logger.error(f"账号索引 {target_account_index} 超出范围，共有 {len(accounts)} 个账号")
                    return None

                account = accounts[target_account_index]
                email = account['email']
                ...
                if not all([email, password, client_id]):
                    logger.error(f"账号 {target_account_index} 配置信息不完整")
                    return None
        ...
        try:
            ...
            try:
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(asyncio.run, load_from_db(account_index))
                    return future.result()
            except RuntimeError:
                # 没有运行中的循环，创建新的
                return asyncio.run(load_from_db(account_index))
        except Exception as e:
    ...
    def add_account(account_string_or_email, password=None, client_id=None, access_token=None, config_file="email_config.json"):
        ...
        async def add_to_db():
            try:
                ...
                # 确定邮箱提供商
                provider = 'outlook' if '@outlook.com' in email else 'hotmail' if '@hotmail.com' in email else 'outlook'
                ...
                """, email, password, client_id, access_token, provider)

                logger.info(f"邮箱账号已保存到数据库: {email}")
                return True

            except Exception as e:
                logger.error(f"添加邮箱账号到数据库失败: {e}")
                return False
            finally:
                if 'conn' in locals():
                    await conn.close()
        ...
        try:
            ...
            try:
                return loop.run_until_complete(add_to_db())
            finally:
                loop.close()
        except Exception as e:
    ...
    def list_accounts(config_file="email_config.json"):
        ...
        try:
            ...
            asyncio.set_event_loop(loop)
            try:
                return loop.run_until_complete(list_from_db())
            finally:
                loop.close()
        except Exception as e:
    ...
    def set_current_account(account_index, config_file="email_config.json"):
        ...
        async def set_current_in_db():
            try:
                ...
                if account_index >= account_count:
                    logger.error(f"账号索引 {account_index} 超出范围，共有 {account_count} 个账号")
                    return False
        ...
        try:
            ...
            asyncio.set_event_loop(loop)
            try:
                return loop.run_until_complete(set_current_in_db())
            finally:
                loop.close()
        except Exception as e:
            logger.error(f"设置当前账号失败: {e}")
    ...
    def get_next_available_account(config_file="email_config.json"):
        ...
        async def get_next_from_db():
            try:
                ...
                # 循环到下一个账号
                next_index = (current_index + 1) % account_count
                ...
                # 获取下一个账户信息
                next_account = await conn.fetchrow("""
                    SELECT email, password, client_id, access_token
                    FROM email_accounts
                    WHERE is_active = true
                    ORDER BY created_at ASC
                ...
                if not next_account:
                    logger.error("无法获取下一个账户信息")
                    return None

                logger.info(f"切换到下一个邮箱账号: {next_account['email']}")

                return EmailManagerFactory.create_outlook_manager(
                    next_account['email'],
                    next_account['password'],
                    next_account['client_id'],
                    next_account['access_token']
                )
        ...
        try:
            ...
            asyncio.set_event_loop(loop)
            try:
                return loop.run_until_complete(get_next_from_db())
            finally:
                loop.close()
        except Exception as e:
            logger.error(f"获取下一个账号失败: {e}")
    ...
    def update_account_tfa_secret(email, tfa_secret, config_file="email_config.json"):
        """在数据库中更新指定邮箱账号的tfa_secret"""
        import asyncio
        import asyncpg

        async def update_tfa_in_db():
            try:
                try:
                    from config import get_db_connection_string
                except ImportError:
                ...
                result = await conn.execute("""
                    ...
                    SET tfa_secret = $2, updated_at = CURRENT_TIMESTAMP
        ...
        try:
            ...
            asyncio.set_event_loop(loop)
            try:
                return loop.run_until_complete(update_tfa_in_db())
            finally:
                loop.close()
        except Exception as e:
            logger.error(f"更新 tfa_secret 失败: {e}")
    ...
    def delete_account(account_index, config_file="email_config.json"):
        ...
        async def delete_from_db():
            try:
                ...
                new_count = account_count - 1
                new_current_index = current_index

                if account_index == current_index:
                    # 删除的是当前账号，设置为第一个账号
                    new_current_index = 0 if new_count > 0 else 0
        ...
        try:
            ...
            asyncio.set_event_loop(loop)
            try:
                return loop.run_until_complete(delete_from_db())
            finally:
                loop.close()
        except Exception as e:
            logger.error(f"删除邮箱账号失败: {e}")
    ...
    def update_account(account_index, email=None, password=None, client_id=None, access_token=None, config_file="email_config.json"):
        ...
        async def update_in_db():
            try:
                ...
                if client_id is not None:
                    update_fields.append(f"client_id = ${param_count}")
                ...
                query = f"""
                    ...
                    WHERE email = ${param_count} AND is_active = true
                """

                result = await conn.execute(query, *params)

                if result == 'UPDATE 1':
                    final_email = email if email is not None else current_email
                    logger.info(f"邮箱账号 {final_email} 更新成功")
                    return True
                else:
                    logger.error("更新账号失败")
                    return False
        ...
        try:
            ...
            asyncio.set_event_loop(loop)
            try:
                return loop.run_until_complete(update_in_db())
            finally:
                loop.close()
        except Exception as e:
            logger.error(f"更新邮箱账号失败: {e}")
